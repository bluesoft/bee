...
class ColumnStructureValidationTest  extends spock.lang.Specification {
	...
	def "it should return an error for each difference between the database and the reference metadata"() {
		...
		new TableColumn(name:'usuario_key' ,autoIncrement:null)				| new TableColumn(autoIncrement:'0') 			| MessageType.AUTO_INCREMENT 				| 'The autoIncrement of the column usuario_key of the table people should be 0 but it is null'
		new TableColumn(name:'ticket_key' ,onUpdateCurrentTimestamp:null)	| new TableColumn(onUpdateCurrentTimestamp:'0') | MessageType.ON_UPDATE_CURRENT_TIMESTAMP  | 'The onUpdateCurrentTimestamp of the column ticket_key of the table people should be 0 but it is null'
	}
	...
	def "it should return no message if there is no differente between the database and reference metadata"() {
		...
		new TableColumn(autoIncrement:false)	| new TableColumn(autoIncrement:false)
	}
}
